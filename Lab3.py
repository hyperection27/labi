...
чем количество отрицательных  элементов в нечетных столбцах в области 4, то поменять в С симметрично области 1 и 3 местами,
иначе С и Е поменять местами несимметрично. При этом матрица А не меняется. После чего вычисляется выражение: (F+A)*AT – K * F
...
for j in range(m//2, m):
    ...
    for i in range(0, m-j):
        count_neg += 1 if c[i][j] < 0 else 0
print("В С количество положительных чисел в четных столбцах в области 2 = ", count_neg)
